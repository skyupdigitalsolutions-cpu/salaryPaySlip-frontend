        ...
        <table style={{
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style={{ ...vc, fontWeight:"400", textAlign:"right" }}>{fmt(ded)}</td>
            </tr>
            <tr>
              <td colSpan="2"></td>
...
export default function Page() {
  ...
  const handlePreview = async () => {
    if (fetchStatus === "notfound" && !isNewJoineeViaModal) { showToast("❌ Employee ID not found."); return; }
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(element, {
        scale: 6, useCORS: true, logging: false,
        ...
        <input
          ...
          className={`w-full px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm border rounded-md focus:outline-none focus:ring-1 transition-colors
            ${formik.touched[name] && formik.errors[name] ? "border-red-500 focus:ring-red-500"
              ...
              : isNewEntry ? "border-blue-400 bg-blue-50 focus:ring-blue-500 placeholder-blue-400"
              : "border-gray-300 focus:ring-indigo-500"}`}
        />
        {formik.touched[name] && formik.errors[name] && (
          <p className="text-red-500 text-[10px] sm:text-xs mt-0.5">{formik.errors[name]}</p>
        )}
      </div>
    );
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="no-print bg-white px-4 md:px-8 py-3 flex items-center gap-4 shadow-md sticky top-0 z-40">
        <div className="flex items-center justify-center flex-shrink-0 bg-white">
          <img src="../../assets/rbd-logo.webp" alt="Skyup Logo" className="h-10 w-auto object-contain" />
        </div>
        ...
        >
          <span className="relative flex-shrink-0">
            ...
            <span className="absolute -top-1 -right-1 w-3 h-3 sm:w-3.5 sm:h-3.5 bg-[#c9a84c] rounded-full flex items-center justify-center">
              <svg className="w-1.5 h-1.5 sm:w-2 sm:h-2 text-white" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4"/>
              </svg>
            </span>
          </span>
          <span className="text-[10px] sm:text-xs font-bold tracking-widest uppercase hidden sm:inline">New Employee</span>
        </button>
        ...
        <div className="flex items-center gap-2 md:gap-3 pl-2 sm:pl-3 border-l border-gray-200">
          {adminInfo && (
            <div className="hidden md:flex flex-col items-end">
              ...
              <span className="text-gray-400 text-[10px] capitalize">{adminInfo.role}</span>
            </div>
          )}
          <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm no-print px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-auto overflow-hidden">
            <div className="bg-[#0037CA] px-4 sm:px-6 py-4 sm:py-5 flex items-center gap-3">
              ...
              <div>
                <h3 className="text-white font-bold text-sm sm:text-base tracking-wide">Add New Employee</h3>
                <p className="text-white/80 text-[10px] sm:text-xs mt-0.5">Enter a unique Employee ID to start manual entry</p>
              </div>
            ...
            <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-4">
              <div>
                ...
                <p className="text-gray-400 text-[10px] sm:text-xs mt-1.5">All personal details will be editable. Employee will be saved to the database on PDF generation.</p>
              </div>
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {["Manual entry for all fields","New Joinee badge on slip","Saved to DB on PDF generation"].map(t => (
                  <span key={t} className="flex items-center gap-1 text-[9px] sm:text-[10px] text-blue-700 bg-blue-50 border border-blue-200 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full font-medium">
            ...
            <div className="px-4 sm:px-6 pb-4 sm:pb-6 flex gap-2 sm:gap-3 justify-end flex-wrap">
              <button type="button" onClick={() => setShowNewEmpModal(false)}
              ...
              <button type="button" onClick={handleNewEmployeeProceed}
      ...
      <div className="no-print w-full px-3 sm:px-6 md:px-8 py-4 sm:py-6 md:py-10">
        ...
        <h2 className="text-base sm:text-xl md:text-2xl font-semibold text-gray-800 mb-4 sm:mb-6 text-center">Salary Slip Form</h2>
          ...
          <div className="bg-white rounded-xl shadow-lg w-full max-w-3xl mx-auto px-3 py-4 sm:px-6 sm:py-6 md:px-10 md:py-8 space-y-4 sm:space-y-6">
            ...
            {isNewJoinee && isNewJoineeViaModal && (
              <div className="flex items-start gap-2 sm:gap-3 bg-[#0037CA] border border-blue-300 rounded-lg px-3 sm:px-4 py-2.5 sm:py-3">
                ...
                <div>
                  <p className="text-white font-semibold text-xs sm:text-sm">New Employee — Manual Entry Mode</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div>
                ...
                {fetchStatus === "found" && !isNewJoinee && (
                  <p className="text-green-600 text-[10px] sm:text-xs mt-0.5">✓ Employee details loaded</p>
                )}
                {isUnknownId && (
                ...
                {isNewJoineeViaModal && (
                  <p className="text-blue-700 text-[10px] sm:text-xs mt-0.5 font-medium">✦ New employee — fill in all details below</p>
                )}
                {formik.touched.employeeId && formik.errors.employeeId && (
                  <p className="text-red-500 text-[10px] sm:text-xs mt-0.5">{formik.errors.employeeId}</p>
                )}
              </div>
              ...
              <div>
                ...
                {formik.touched.payMonth && formik.errors.payMonth && (
                  <p className="text-red-500 text-[10px] sm:text-xs mt-0.5">{formik.errors.payMonth}</p>
                )}
              </div>
            </div>

            {/* ── Auto-filled Fields ── */}
            <div className={`grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 ${isUnknownId ? "opacity-50 pointer-events-none select-none" : ""}`}>
            ...
            <div>
              ...
              {formik.touched.email && formik.errors.email && (
                <p className="text-red-500 text-[10px] sm:text-xs mt-0.5">{formik.errors.email}</p>
              )}
              {formik.values.email && (
                <p className="text-blue-500 text-[10px] sm:text-xs mt-0.5">📧 Salary slip will be sent to this address on PDF generation</p>
              )}
            </div>

            {/* ── Attendance ── */}
            <div className={isUnknownId ? "opacity-50 pointer-events-none select-none" : ""}>
              <h3 className="text-xs sm:text-sm font-semibold text-gray-800 mb-1.5 sm:mb-2">Attendance</h3>
            ...
            {/* ── Earnings ── */}
            <div className={isUnknownId ? "opacity-50 pointer-events-none select-none" : ""}>
              <h3 className="text-xs sm:text-sm font-semibold text-gray-800 mb-1.5 sm:mb-2">Earnings</h3>
            ...
            {/* ── Deductions ── */}
            <div className={isUnknownId ? "opacity-50 pointer-events-none select-none" : ""}>
              <h3 className="text-xs sm:text-sm font-semibold text-gray-800 mb-1.5 sm:mb-2">Deductions</h3>
              <div>
                <label className="block text-[10px] sm:text-xs font-medium text-gray-700 mb-0.5 sm:mb-1">
                  Loss of Pay (₹)
                ...
                <input
                  type="number" name="lossOfPay" value={formik.values.lossOfPay} readOnly
                ...
                {formik.touched.lossOfPay && formik.errors.lossOfPay && (
                  <p className="text-red-500 text-[10px] sm:text-xs mt-0.5">{formik.errors.lossOfPay}</p>
                )}
              </div>
            </div>

            {/* ── Net Salary Summary ── */}
            <div className={`border rounded-md p-2.5 sm:p-3 ${net > 0 ? "bg-blue-50 border-blue-200" : "bg-gray-50 border-gray-200"}`}>
              <div className={`text-xs sm:text-sm font-semibold ${net > 0 ? "text-blue-800" : "text-gray-600"}`}>
                Net Salary: ₹{net.toLocaleString("en-IN")}
              </div>
              <div className={`text-[10px] sm:text-xs mt-0.5 ${net > 0 ? "text-blue-600" : "text-gray-500"}`}>
        ...
        {/* ── Inline Preview ── */}
        {showPreview && (
          <div ref={previewRef} className="mt-6 sm:mt-8 bg-white rounded-xl p-3 sm:p-4 md:p-6 w-full max-w-4xl mx-auto">

            {/* Header row */}
            <div className="flex items-center justify-between mb-3 sm:mb-4 flex-wrap gap-2">
              <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                <h3 className="text-sm sm:text-base md:text-lg font-semibold text-gray-800">Salary Slip Preview</h3>
                {isNewJoinee && isNewJoineeViaModal && (
            ...
            {/*
              On mobile: scale down to ~45% so the 794px slip fits a ~360px screen.
              On sm+: scale to 75% as before.
              Outer div clips to scaled height so no extra whitespace below.
            */}
            <div style={{
              width: "100%",
              maxWidth: "595px",
              ...
              <div style={{
                ...
                transform: "scale(var(--slip-scale, 0.45))",
                backgroundColor: "#ffffff",
              }}
              className="slip-scale-wrapper"
              >
                <SlipContent values={formik.values} isNewJoinee={isNewJoinee} />
              </div>
            </div>

            {/* Responsive scale via inline style tag */}
            <style>{`
              .slip-scale-wrapper { transform: scale(0.42); }
              @media (min-width: 400px) { .slip-scale-wrapper { transform: scale(0.48); } }
              @media (min-width: 500px) { .slip-scale-wrapper { transform: scale(0.60); } }
              @media (min-width: 640px) { .slip-scale-wrapper { transform: scale(0.75); } }
            `}</style>
          </div>
        )}

        {/* ── Hidden PDF capture — same dimensions, same component, same props ── */}
        <div
      ...
      {/* ── Toast ── */}
      {toast && (
        <div className={`fixed bottom-4 sm:bottom-6 right-3 left-3 sm:right-4 sm:left-4 md:left-auto md:right-6 z-50 flex items-center gap-2 sm:gap-3 px-3 sm:px-5 py-2.5 sm:py-3.5 rounded-xl text-xs sm:text-sm font-medium text-white shadow-xl ${toast.type === "error" ? "bg-red-600" : "bg-[#1a3a2a]"}`}>
          <span className="flex-1">{toast.msg}</span>
          <button onClick={() => setToast(null)} className="ml-1 sm:ml-2 opacity-60 hover:opacity-100 text-lg leading-none flex-shrink-0">×</button>
        </div>
      )}
    </div>
  );
